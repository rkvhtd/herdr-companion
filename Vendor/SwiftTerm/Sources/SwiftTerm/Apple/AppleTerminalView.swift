...
extension TerminalView {
    ...
    {
        resetCaches()
        let oldCellHeight = cellDimension.height
        self.cellDimension = computeFontDimensions ()
        #if os(iOS) || os(visionOS)
        // herdr: a reader parked mid-row keeps their fraction of a row, not the
        // old font's pixel amount.
        rescaleManualScrollOffsetWithinRow (oldCellHeight: oldCellHeight)
        #endif
        if (frame.width > 0) && (frame.height > 0) {
            let newCols = Int(frame.width / cellDimension.width)
            let newRows = Int(frame.height / cellDimension.height)
            // herdr: a font change *requests* a grid. It must not soft-reset the
            // terminal (which the old `resize` did), and with managed geometry it
            // must not reflow before the owner commits.
            requestTerminalSize (cols: newCols, rows: newRows)
        }
        updateCaretView()
    ...
    func processSizeChange (newSize: CGSize) -> Bool {
        ...
        return requestTerminalSize (cols: newCols, rows: newRows)
    }

    /// A layout or font change wants this grid.
    ///
    /// herdr: when an embedder owns the geometry (`sizeChangeRequestHandler`),
    /// this only *publishes* the proposal - the emulator is not resized, the
    /// selection is not cleared and no reflow happens - because the authoritative
    /// grid arrives later, in band with the output it applies to. Without such an
    /// embedder the view keeps fitting itself, through the commit path below.
    ///
    /// Returns true only when the emulator's grid actually changed.
    @discardableResult
    func requestTerminalSize (cols: Int, rows: Int) -> Bool {
        guard cols > 0, rows > 0 else {
            // A zero-sized layout pass says nothing about what the user wants;
            // it must not replace the last valid target.
            return false
        }
        #if os(iOS) || os(visionOS)
        if let sizeChangeRequestHandler {
            let cellSize = CGSize (width: cellDimension.width, height: cellDimension.height)
            if let requested = requestedTerminalGrid, requested.cols == cols, requested.rows == rows,
               requestedTerminalCellSize == cellSize {
                return false
            }
            requestedTerminalGrid = (cols: cols, rows: rows)
            requestedTerminalCellSize = cellSize
            sizeChangeRequestHandler (cols, rows)
            return false
        }
        #endif
        return commitTerminalSize (cols: cols, rows: rows, notifyDelegate: true)
    }

    /// Commits a grid to the emulator: reflow happens here and nowhere else.
    ///
    /// herdr: unlike the old `resize(cols:rows:)` this never soft-resets the
    /// terminal, so modes, margins and installed colors survive a resize. Calling
    /// it with the current grid is legal and cheap: it reconciles the scroller and
    /// cursor against current font metrics and queues a safe repaint without
    /// touching the buffers.
    public func applyTerminalSize (cols: Int, rows: Int)
    {
        commitTerminalSize (cols: cols, rows: rows, notifyDelegate: false)
    }

    @discardableResult
    func commitTerminalSize (cols: Int, rows: Int, notifyDelegate: Bool) -> Bool
    {
        let changed = cols != terminal.cols || rows != terminal.rows
        if changed {
        ...
        // A same-grid commit is also the no-output repaint path. Mark rows dirty;
        // queueing alone is a no-op once the previous update range was consumed.
    ...
    public func synchronizedOutputChanged (source: Terminal, active: Bool)
    {
        ...
        #if os(iOS) || os(visionOS)
        // herdr: an embedder holding a frame during a resize needs the batch
        // boundaries; this runs after the view's own bookkeeping.
        synchronizedOutputChangeHandler? (active)
        #endif
    }
    ...
    public func scrollTo (row: Int, notifyAccessibility: Bool = true)
    {
        ...
        // herdr: an explicit scroll is the user (or assistive paging) choosing a
        // row, so the logical anchor kept for resizing no longer describes what
        // they are reading. Ordinary offset reconciliation must not do this.
    ...
    {
        // herdr: this used to `terminal.softReset()` afterwards, which threw away
        // bracketed paste, origin/application-cursor modes and installed colors
        // on every window resize. A caller asking for a grid asked for a grid.
        commitTerminalSize (cols: cols, rows: rows, notifyDelegate: true)
    }
