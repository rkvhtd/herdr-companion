...
public final class Buffer {
    ...
    // MARK: - herdr: logical viewport anchoring across reflow
    //
    // Reflow rewrites the physical row layout, so a scrolled-back viewport that
    // is expressed as a row index (or a pixel offset, or a `totalLinesTrimmed`
    // delta) silently changes which text the reader is looking at: widening
    // 80 -> 120 columns collapses wrapped rows, and the old `yDisp` then points
    // at completely different content. Instead we pin one logical cell - the
    // first content cell of the top visible row - and let reflow's own cell
    // copies carry it to wherever that text ends up.
    private enum ResizeViewportAnchor {
        /// The reader is following the tail: the newest output must stay visible.
        case tail
        /// The reader is in the scrollback: this cell must stay in the top row.
        case history (line: BufferLine, column: Int)
    }

    /// Current home of the anchored cell. `reflowCopy` moves it as the existing
    /// reflow copies move the cell. Non-nil only while a resize is in progress.
    private var trackedAnchorLine: BufferLine? = nil
    private var trackedAnchorColumn: Int = 0
    /// Set when reflow cleared the anchored cell: its line still identifies the
    /// reading position, its exact column no longer does.
    private var trackedAnchorColumnLost: Bool = false

    /// Anchor carried across *successive* resizes, so a repeated 80->120->80
    /// sweep does not re-anchor onto a different logical cell each time (the
    /// first content cell of the top row is not necessarily the same cell after
    /// a rewrap). Only valid while that line is still the top visible history
    /// line and has not been mutated since (hence the generation).
    private var cachedAnchorLine: BufferLine? = nil
    private var cachedAnchorColumn: Int = 0
    private var cachedAnchorGeneration: UInt64 = 0
...
//                #endif
    ...
    {
        let resizeAnchor = captureResizeViewportAnchor ()
        ...
        rows = newRows
        cols = newCols
        restoreResizeViewport (resizeAnchor)
    }

    /// Records what the reader is looking at. Must run before any structural
    /// mutation of this buffer: line widening, row insertion/removal, scrollback
    /// trimming or reflow.
    private func captureResizeViewportAnchor () -> ResizeViewportAnchor
    {
        trackedAnchorLine = nil
        trackedAnchorColumn = 0
        trackedAnchorColumnLost = false
        ...
        // circular buffer recycled into the tail after our anchor was evicted.
        if let cached = cachedAnchorLine, cached === top,
           cached.generation == cachedAnchorGeneration,
           cachedAnchorColumn < cached.count,
           cachedAnchorColumn == 0 || cachedAnchorColumn < cached.getTrimmedLength () {
            trackedAnchorLine = cached
            trackedAnchorColumn = cachedAnchorColumn
            return .history (line: cached, column: cachedAnchorColumn)
        }
        cachedAnchorLine = nil

        var anchorLine = top
        var anchorColumn = firstContentColumn (top)
        if anchorColumn == nil && top.isWrapped {
            // An all-null wrapped continuation is exactly the row reflow merges
            // away. Anchoring on it would look like eviction and teleport the
            // reader to the oldest history, so anchor on the nearest preceding
            // content cell of its own wrapped group instead.
            var i = _yDisp - 1
            var groupHead = top
            while i >= 0 {
                let candidate = lines [i]
                groupHead = candidate
                if let column = lastContentColumn (candidate) {
                    anchorLine = candidate
                    anchorColumn = column
                    break
                }
                if !candidate.isWrapped {
                    // Reached the head of the group and it is blank too.
                    break
                }
                i -= 1
            }
            if anchorColumn == nil {
                anchorLine = groupHead
            }
        }

        let column = anchorColumn ?? 0
        trackedAnchorLine = anchorLine
        trackedAnchorColumn = column
        return .history (line: anchorLine, column: column)
    }

    /// Puts the viewport back on the anchored text once the new geometry, the
    /// row insertions/removals and the final `rows`/`cols` are all in place.
    private func restoreResizeViewport (_ anchor: ResizeViewportAnchor)
    {
        defer {
            trackedAnchorLine = nil
            trackedAnchorColumn = 0
        ...
        case .tail:
            ...
            yDisp = yBase
        case .history:
            guard let destination = trackedAnchorLine else {
                // No line to go back to: keep the clamped viewport rather than
                // teleporting the reader anywhere.
                cachedAnchorLine = nil
                yDisp = min (max (_yDisp, 0), yBase)
                return
            }
            var row = -1
            for i in 0..<lines.count {
            ...
            guard row >= 0 else {
                // The anchored line itself is gone - the scrollback cap or the
                // height change actually dropped it. Oldest surviving history,
                // without turning tail-following back on.
                cachedAnchorLine = nil
                yDisp = 0
                return
            }

            var column = min (max (trackedAnchorColumn, 0), max (destination.count - 1, 0))
            let occupied = destination.getTrimmedLength ()
            if trackedAnchorColumnLost || column >= occupied {
                // Truncated at the new right edge, or turned into padding: the
                // line still holds the reading position, so use the nearest
                // remaining content cell on it (column zero if it is empty).
                column = occupied == 0 ? 0 : min (column, occupied - 1)
                while column > 0 && destination [column].code == 0 {
                    column -= 1
                }
            }

            let clamped = min (row, yBase)
            yDisp = clamped
            if clamped == row && row < yBase {
                cachedAnchorLine = destination
                cachedAnchorColumn = column
                cachedAnchorGeneration = destination.generation
            } else {
                cachedAnchorLine = nil
            }
        }
    ...
    /// Copies cells exactly like `BufferLine.copyFrom`, but first moves the
    /// tracked anchor if it is one of the cells being moved. Source and
    /// destination may be the same line, and a cell may legitimately be moved
    /// more than once during one reflow.
    @inline(__always)
    private func reflowCopy (_ dest: BufferLine, _ src: BufferLine, srcCol: Int, dstCol: Int, len: Int)
    {
        if len > 0, let tracked = trackedAnchorLine, tracked === src,
           trackedAnchorColumn >= srcCol, trackedAnchorColumn < srcCol + len {
            trackedAnchorLine = dest
            trackedAnchorColumn = dstCol + (trackedAnchorColumn - srcCol)
            trackedAnchorColumnLost = false
        }
        dest.copyFrom (src, srcCol: srcCol, dstCol: dstCol, len: len)
    }

    /// Clears cells exactly like `BufferLine.replaceCells`, noting whether the
    /// anchored cell was one of them. Cells that were copied elsewhere first
    /// have already moved the anchor, so vacating their source is not a loss.
    @inline(__always)
    private func reflowClear (_ line: BufferLine, start: Int, end: Int, fillData: CharData)
    {
        noteReflowCellsCleared (line, start: start, end: end)
        line.replaceCells (start: start, end: end, fillData: fillData)
    }

    @inline(__always)
    private func noteReflowCellsCleared (_ line: BufferLine, start: Int, end: Int)
    {
        if let tracked = trackedAnchorLine, tracked === line,
           trackedAnchorColumn >= start, trackedAnchorColumn < end {
            trackedAnchorColumnLost = true
        }
    }

    /// First cell of the line that holds a character, or nil for a blank line.
    private func firstContentColumn (_ line: BufferLine) -> Int?
    {
    ...
    /// Last cell of the line that holds a character, or nil for a blank line.
    private func lastContentColumn (_ line: BufferLine) -> Int?
    {
        let trimmed = line.getTrimmedLength ()
        return trimmed == 0 ? nil : trimmed - 1
    }

    /// Drops the anchor carried across resizes. The view calls this when the
    /// *user* moves the viewport (a scroll gesture, or an explicit `scrollTo`):
    /// the reader is looking at something else now, so the next resize must
    /// capture a fresh anchor instead of restoring this one.
    func invalidateResizeViewportAnchor ()
    {
        cachedAnchorLine = nil
    }
    ...
    func getLinesToRemove (oldCols: Int, newCols: Int, bufferAbsoluteY: Int, nullChar: CharData) -> [Int]
    {
        ...
        while y < lines.count-1 {
            ...
            var destLineIndex = 0
            // wrappedLines, not lines: `lines [0]` is row zero of the whole
            // buffer, not the head of the group being reflowed here.
            ...
            while srcLineIndex < wrappedLines.count {
                ...
                if destLineIndex < wrappedLines.count {
                    reflowCopy (wrappedLines [destLineIndex], wrappedLines [srcLineIndex], srcCol: srcCol,
                ...
                if destCol == 0 && destLineIndex != 0 {
                    if wrappedLines [destLineIndex - 1].getWidth(index: newCols - 1) == 2 {
                        reflowCopy (wrappedLines [destLineIndex], wrappedLines [destLineIndex - 1], srcCol: newCols - 1, dstCol: destCol, len: 1)
                        destCol += 1
                        // Null out the end of the last row
                        reflowClear (wrappedLines [destLineIndex - 1], start: newCols - 1, end: newCols, fillData: nullChar)
                    }
                }
            }

            // Clear out remaining cells or fragments could remain;
            reflowClear (wrappedLines [destLineIndex], start: destCol, end: newCols, fillData: nullChar)
    ...
    func reflowNarrower (_ oldCols: Int, _ oldRows: Int, _ newCols: Int, _ newRows: Int)
    {
        ...
        while y >= 0 {
            ...
            while srcLineIndex >= 0 {
                ...
                reflowCopy (wrappedLines [destLineIndex], wrappedLines [srcLineIndex], srcCol: srcCol - cellsToCopy, dstCol: destCol - cellsToCopy, len: cellsToCopy)
                destCol -= cellsToCopy
                if destCol == 0 {
                    destLineIndex -= 1
                    if destLineIndex >= 0 {
            ...
            for i in 0..<wrappedLines.count {
                if destLineLengths [i] < newCols {
                    noteReflowCellsCleared (wrappedLines [i], start: destLineLengths [i], end: destLineLengths [i] + 1)
