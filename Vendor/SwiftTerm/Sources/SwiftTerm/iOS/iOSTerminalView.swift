...
/// A key produced by an embedder-owned control surface. `TerminalView` encodes
/// these through the same Kitty/application-cursor machinery as hardware keys.
public enum TerminalKeyPress: Equatable {
    case escape, tab, backTab, enter
    case up, down, left, right
    case home, end, pageUp, pageDown, deleteForward
    case function(Int)
    case text(String)
}

public struct TerminalKeyPressModifiers: OptionSet, Equatable {
    public let rawValue: Int

    public init(rawValue: Int) { self.rawValue = rawValue }

    public static let shift = TerminalKeyPressModifiers(rawValue: 1 << 0)
    public static let option = TerminalKeyPressModifiers(rawValue: 1 << 1)
    public static let control = TerminalKeyPressModifiers(rawValue: 1 << 2)
}
...
open class TerminalView: UIScrollView, UITextInputTraits, UIKeyInput, UIScrollViewDelegate, TerminalDelegate, UIPointerInteractionDelegate {
    ...
    /// Hardware Page Up/Down normally move SwiftTerm's local history. A remote
    /// direct-attach embedder can turn this off so the negotiated key event is
    /// sent to the attach client, which owns remote history routing.
    public var pageKeysScrollLocally = true
    ...
    // MARK: - herdr: managed geometry and presentation hooks
    //
    // An embedder that owns the terminal's geometry (because a remote pty, not
    // this view, decides the grid) installs `sizeChangeRequestHandler`: layout
    // and font changes then *propose* a grid instead of reflowing the emulator
    // behind the embedder's back. `applyTerminalSize` is the matching commit.

    /// Called with the (cols, rows) this view's pixel size wants. While it is
    /// installed, `processSizeChange`/`resetFont` never resize the emulator.
    public var sizeChangeRequestHandler: ((Int, Int) -> Void)?

    /// Called when DEC 2026 synchronized output begins (true) or ends (false).
    public var synchronizedOutputChangeHandler: ((Bool) -> Void)?

    /// Called after every actual CoreText paint. The flag is false when the
    /// paint happened mid-synchronized-batch, i.e. the view's backing image is
    /// not a complete frame and must not be treated as one.
    public var displayCompletedHandler: ((Bool) -> Void)?

    /// Actual pixel-snapped cell metrics in view points, including line spacing.
    public var cellSize: CGSize {
        CGSize(width: cellDimension.width, height: cellDimension.height)
    }

    /// Last proposed grid plus the cell metrics it was computed from, so an
    /// unchanged layout does not re-propose, but a font change does.
    var requestedTerminalGrid: (cols: Int, rows: Int)? = nil
    var requestedTerminalCellSize: CGSize = .zero

    /// Normal-buffer reading intent, kept across alternate-screen borrowing.
    var savedNormalScrollIntent: (userScrolling: Bool, offsetWithinRow: CGFloat)? = nil
    weak var savedNormalScrollBuffer: Buffer? = nil
    ...
    public var font: UIFont {
        ...
        set {
            ...
            resetFont()
            // herdr: with managed geometry the grid commit invalidates the
            // selection; dropping it here would also drop it for a font change
            // that does not change the grid at all.
            if sizeChangeRequestHandler == nil {
                selectNone()
            }
        }
    }
    ...
    /// Sends one complete paste transaction. Keeping the bracket markers and
    /// payload in one delegate call is important for direct-attach parsers,
    /// which classify a complete bracketed paste as one semantic input event.
    public func sendPaste(_ text: String) {
        terminalAccessory?.controlModifier = false
        controlModifier = false
        metaModifier = false
        var bytes: [UInt8] = []
        if terminal.bracketedPasteMode { bytes += EscapeSequences.bracketedPasteStart }
        bytes += text.utf8
        if terminal.bracketedPasteMode { bytes += EscapeSequences.bracketedPasteEnd }
        send(bytes)
        queuePendingDisplay()
    }

    /// Encodes an embedder key using the active terminal keyboard protocol and
    /// cursor mode, then consumes any armed one-shot Ctrl/Option state.
    public func sendKeyPress(
        _ key: TerminalKeyPress,
        modifiers explicitModifiers: TerminalKeyPressModifiers = []
    ) {
        var modifiers = explicitModifiers
        if (terminalAccessory?.controlModifier ?? false) || controlModifier {
            modifiers.insert(.control)
        }
        if metaModifier { modifiers.insert(.option) }
        if key == .backTab { modifiers.insert(.shift) }

        var kittyModifiers: KittyKeyboardModifiers = []
        if modifiers.contains(.shift) { kittyModifiers.insert(.shift) }
        if modifiers.contains(.option) { kittyModifiers.insert(.alt) }
        if modifiers.contains(.control) { kittyModifiers.insert(.ctrl) }

        let kittyKey: KittyKey
        var text: String?
        switch key {
        case .escape: kittyKey = .functional(.escape)
        case .tab, .backTab: kittyKey = .functional(.tab)
        case .enter: kittyKey = .functional(.enter)
        case .up: kittyKey = .functional(.up)
        case .down: kittyKey = .functional(.down)
        case .left: kittyKey = .functional(.left)
        case .right: kittyKey = .functional(.right)
        case .home: kittyKey = .functional(.home)
        case .end: kittyKey = .functional(.end)
        case .pageUp: kittyKey = .functional(.pageUp)
        case .pageDown: kittyKey = .functional(.pageDown)
        case .deleteForward: kittyKey = .functional(.delete)
        case .function(let number):
            guard let function = Self.syntheticFunctionKey(number) else { return }
            kittyKey = .functional(function)
        case .text(let value):
            guard value.unicodeScalars.count == 1, let scalar = value.unicodeScalars.first else { return }
            let base = String(scalar).lowercased().unicodeScalars.first ?? scalar
            kittyKey = .unicode(base.value)
            if !modifiers.contains(.control), !modifiers.contains(.option) { text = value }
        }
        ...
        guard let press = encoder.encode(event) else { return }
        var bytes = press
        if terminal.keyboardEnhancementFlags.contains(.reportEvents) {
            var release = event
            release.eventType = .release
            release.text = nil
            if let encodedRelease = encoder.encode(release) {
                bytes += encodedRelease
            }
        }
        terminalAccessory?.controlModifier = false
        controlModifier = false
    ...
    private static func syntheticFunctionKey(_ number: Int) -> KittyFunctionalKey? {
        switch number {
        case 1: return .f1
        case 2: return .f2
        case 3: return .f3
    ...
    func encodeFlags (release: Bool) -> Int
    {
        let encodedFlags = terminal.encodeButton(
            ...
            shift: false,
            meta: false,
            control: (terminalAccessory?.controlModifier ?? false) || controlModifier)
        terminalAccessory?.controlModifier = false
        controlModifier = false
        return encodedFlags
    }
    ...
    open func bufferActivated(source: Terminal) {
        // herdr: an application borrowing the alternate screen (an editor, a
        // full-screen redraw during a resize) must not cost a reader their place
        // in the scrollback. The alternate screen gets its own top-zero
        // viewport; the normal buffer's reading intent is put back on return.
        if source.isDisplayBufferAlternate {
            if savedNormalScrollIntent == nil {
                savedNormalScrollIntent = (userScrolling, manualScrollOffsetWithinRow)
                savedNormalScrollBuffer = source.normalBuffer
            }
            resetManualScrollTracking()
        } else if let intent = savedNormalScrollIntent, savedNormalScrollBuffer === source.normalBuffer {
            // Same normal buffer we left: restore the reader's intent. `yDisp`
            // itself was never touched by the alternate screen.
            savedNormalScrollIntent = nil
            savedNormalScrollBuffer = nil
            setManualScrolling(intent.userScrolling)
            if intent.userScrolling {
                manualScrollOffsetWithinRow = intent.offsetWithinRow
            } else {
                terminal.setViewYDisp(maxDisplayRow(in: source.displayBuffer))
            }
        } else {
            // A reset or a replaced normal buffer: the old intent is meaningless.
            savedNormalScrollIntent = nil
            savedNormalScrollBuffer = nil
            resetManualScrollTracking()
        }
    ...
    func updateScroller ()
    {
        // herdr: every write below is this view reconciling itself with the
        // emulator, never a user gesture. The flag is restored rather than
        // cleared so a nested reconcile cannot unmask an outer one.
        let wasUpdatingFromTerminal = updatingContentOffsetFromTerminal
    ...
    /// Keeps a scrolled-back reader's sub-row position when the cell height
    /// changes: the fraction of a row is meaningful, the old pixel amount is not.
    /// Must be called after `cellDimension` has been replaced.
    func rescaleManualScrollOffsetWithinRow (oldCellHeight: CGFloat) {
        guard oldCellHeight > 0, cellDimension.height > 0, manualScrollOffsetWithinRow != 0 else {
            return
        }
        let fraction = min (max (manualScrollOffsetWithinRow / oldCellHeight, 0), 1)
        manualScrollOffsetWithinRow = fraction * cellDimension.height
    }
    ...
    private func syncYDispFromContentOffset() {
        ...
        if offsetY >= maxContentOffset - atBottomThreshold {
            // herdr: only a real gesture re-engages auto-follow. UIKit clamps
            // the offset on its own during a frame/bounds change (a resize, a
            // keyboard inset), and treating that as "the user went to the
            // bottom" silently cancelled a reader's position.
            guard isTracking || isDecelerating else {
                return
            }
            if displayBuffer.yDisp != maxRow {
                terminal.setViewYDisp(maxRow)
                displayBuffer.invalidateResizeViewportAnchor()
            }
            setManualScrolling(false)
            return
        }
        ...
        if displayBuffer.yDisp != row {
            ...
            // The user chose this row: the logical anchor from an earlier resize
            // is no longer what they are reading.
            displayBuffer.invalidateResizeViewportAnchor()
        }
    ...
    override public func draw (_ dirtyRect: CGRect) {
        ...
        // herdr: report that a paint actually happened, and whether it can be
        // trusted as a whole frame. UIKit forces partial paints during layout
        // and scrolling, so an embedder that retains this view's backing image
        // needs to know a mid-batch paint is not a finished frame. Painting is
        // never skipped here: returning early would blank the view.
        displayCompletedHandler? (!terminal.synchronizedOutputActive)
    }
    ...
    open override func accessibilityScroll(_ direction: UIAccessibilityScrollDirection) -> Bool {
        // herdr: page through the *terminal's* rows via the explicit scroll API
        // instead of writing contentOffset and hoping it is read back as a
        // gesture: assistive paging is a deliberate user scroll and must both
        // freeze auto-follow and drop the resize anchor, which a programmatic
        // offset write no longer does.
        let displayBuffer = terminal.displayBuffer
        let pageRows = max (1, Int (max (bounds.height, cellDimension.height) / cellDimension.height))
        let maxRow = maxDisplayRow (in: displayBuffer)
        let targetRow: Int

        switch direction {
        case .down, .right, .next:
            targetRow = min (maxRow, displayBuffer.yDisp + pageRows)
        case .up, .left, .previous:
            targetRow = max (0, displayBuffer.yDisp - pageRows)
        default:
            return super.accessibilityScroll(direction)
        }

        guard targetRow != displayBuffer.yDisp else {
            return false
        }

        scrollTo (row: targetRow)
    ...
    private func commitTextInput(_ text: String, applyModifiers: Bool) {
        ...
        } else if applyModifiers && ((terminalAccessory?.controlModifier ?? false) || controlModifier) {
            let controlBytes = applyControlToEventCharacters(textToInsert)
            if metaModifier { self.send([0x1b] + controlBytes) }
            else { self.send(controlBytes) }
            terminalAccessory?.controlModifier = false
            controlModifier = false
            metaModifier = false
    ...
    */
    ...
    var oneShotControlArmed: Bool {
        (terminalAccessory?.controlModifier ?? false) || controlModifier
    }

    /// Spends only the embedder-owned one-shot state. A physically held Ctrl is
    /// represented by `UIKey.modifierFlags`, so clearing this flag cannot release it.
    private func consumeOneShotControl() {
        guard oneShotControlArmed else { return }
        terminalAccessory?.controlModifier = false
        controlModifier = false
    }
    ...
    private func kittyModifiers(from key: UIKey, includeOption: Bool) -> KittyKeyboardModifiers {
        ...
        if (includeOption && key.modifierFlags.contains(.alternate)) || metaModifier {
    ...
    private func syntheticKeyPress(for keyCode: UIKeyboardHIDUsage) -> TerminalKeyPress? {
        switch keyCode {
    ...
    private func sendKittyTextInput(_ text: String, applyModifiers: Bool) {
        ...
        let controlActive = applyModifiers && ((terminalAccessory?.controlModifier ?? false) || controlModifier)
    ...
    private var oneShotKittyReleases: [UIKeyboardHIDUsage: KittyKeyEvent] = [:]

    private func drainOneShotKittyReleases(for presses: Set<UIPress>? = nil) {
        let keyCodes = presses.map { Set($0.compactMap { $0.key?.keyCode }) }
        let releases = oneShotKittyReleases.filter { keyCodes?.contains($0.key) ?? true }
        for (keyCode, var release) in releases {
            oneShotKittyReleases.removeValue(forKey: keyCode)
            if terminal.keyboardEnhancementFlags.contains(.reportEvents) {
                release.eventType = .release
                release.text = nil
                _ = sendKittyEvent(release)
            }
        }
    }
        ...
        for press in presses {
            ...
            if !kittyFlags.isEmpty {
                ...
                if let functionKey = kittyFunctionalKey(for: key.keyCode) {
                    ...
                    if pageKeysScrollLocally,
                       (functionKey == .pageUp || functionKey == .pageDown),
                    ...
                    let includeOption = optionAsMetaKey || functionKey == .leftAlt || functionKey == .rightAlt
                    let usedOneShotControl = oneShotControlArmed
                    let usedOneShotMeta = metaModifier
                    let modifiers = kittyModifiers(from: key, includeOption: includeOption)
                    ...
                    if sendKittyEvent(pressEvent) {
                        ...
                        keyRepeat?.invalidate()
                        if !isModifierKey {
                            if usedOneShotControl { consumeOneShotControl() }
                            if usedOneShotMeta { metaModifier = false }
                        }
                        if (usedOneShotControl || usedOneShotMeta), !isModifierKey {
                            if kittyFlags.contains(.reportEvents) {
                                oneShotKittyReleases[key.keyCode] = pressEvent
                            }
                            continue
                        }
                        if !isModifierKey {
                ...
                if key.modifierFlags.contains(.control) || oneShotControlArmed
                    || (optionAsMetaKey && key.modifierFlags.contains(.alternate)) || metaModifier {
                    let usedOneShotControl = oneShotControlArmed
                    let usedOneShotMeta = metaModifier
                    if let kittyEvent = kittyTextEvent(from: key, eventType: .press),
                       sendKittyEvent(kittyEvent) {
                        ...
                        let wasOneShot = usedOneShotControl || usedOneShotMeta
                        if usedOneShotControl { consumeOneShotControl() }
                        if usedOneShotMeta { metaModifier = false }
                        if wasOneShot {
                            if kittyFlags.contains(.reportEvents) {
                                oneShotKittyReleases[key.keyCode] = kittyEvent
                            }
                            didHandleEvent = true
                            keyRepeat?.invalidate()
                            continue
            ...
            // Modified hardware special keys and embedder one-shots share the
            // same encoder as the on-screen controls. This avoids legacy
            // Option-Arrow taking a different path from an armed Option + Arrow.
            let usedOneShot = oneShotControlArmed || metaModifier
            let hasPhysicalTerminalModifier = key.modifierFlags.contains(.alternate)
                || key.modifierFlags.contains(.control)
            if (usedOneShot || hasPhysicalTerminalModifier),
               let synthetic = syntheticKeyPress(for: key.keyCode) {
                var modifiers: TerminalKeyPressModifiers = []
                if key.modifierFlags.contains(.shift) { modifiers.insert(.shift) }
                ...
                sendKeyPress(synthetic, modifiers: modifiers)
                didHandleEvent = true
                keyRepeat?.invalidate()
                if !usedOneShot {
                    keyRepeat = Timer(fire: Date(timeInterval: 0.4, since: Date()),
                                      interval: 0.1,
                                      repeats: true) { _ in
                        self.sendKeyPress(synthetic, modifiers: modifiers)
                    }
                    RunLoop.current.add(keyRepeat!, forMode: .default)
                }
                continue
            ...
            case .keyboardPageUp:
                if terminal.applicationCursor || !pageKeysScrollLocally {
            ...
            default:
                if key.modifierFlags.contains ([.alternate, .command]) && key.charactersIgnoringModifiers == "o" {
                    ...
                    let usedOneShotControl = oneShotControlArmed
                    let usedOneShotMeta = metaModifier
                    let controlActive = key.modifierFlags.contains(.control) || usedOneShotControl
                    if controlActive {
                    ...
                    metaModifier = false
                    if usedOneShotControl || usedOneShotMeta { oneShotEncodedKey = true }
                    ...
                    if !controlBytes.isEmpty {
                        ...
                        if usedOneShotControl {
                            consumeOneShotControl()
                            oneShotEncodedKey = true
                        }
                    }
                }
            }
    ...
    public override func pressesEnded(_ presses: Set<UIPress>, with event: UIPressesEvent?) {
        ...
        let flags = terminal.keyboardEnhancementFlags
        let oneShotKeyCodes = Set(presses.compactMap { press -> UIKeyboardHIDUsage? in
            guard let keyCode = press.key?.keyCode,
                  oneShotKittyReleases[keyCode] != nil else { return nil }
            return keyCode
        })
        drainOneShotKittyReleases(for: presses)
        if flags.contains(.reportEvents) {
            for press in presses {
                guard let key = press.key else { continue }
                if oneShotKeyCodes.contains(key.keyCode) {
                    continue
                }
